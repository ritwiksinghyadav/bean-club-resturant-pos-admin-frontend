...
export default function VariantClient({
  ...
  const handleAddSubmit = async (values: VariantFormValues) => {
    ...
    try {
      ...
      const res = await fetchWithAdminAuth(`${API_URL}/admin/variants`, {
        method: 'POST',
        headers: {
  ...
  const handleEditSubmit = async (values: VariantFormValues) => {
    ...
    try {
      ...
      const res = await fetchWithAdminAuth(
        `${API_URL}/admin/variants/${editingVariant.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
      ...
      if (res.ok && data.success) {
        ...
        setVariants((prev) =>
          prev.map((v) =>
            v.id === editingVariant.id ? data.result.variant : v
          )
        );
        setIsEditOpen(false);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetchWithAdminAuth(
        `${API_URL}/admin/variants/${deletingVariantId}`,
        {
          method: 'DELETE'
        }
      );
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground text-sm'>
            Manage global master sizing types (e.g. Regular, Large, XL, Grande)
            for the menu.
          </p>
        </div>
        <Button
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder='e.g. Grande / XL / Regular'
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. Removing this global sizing variant
              will automatically unmap it and delete its pricing configuration
              from all menu items.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
