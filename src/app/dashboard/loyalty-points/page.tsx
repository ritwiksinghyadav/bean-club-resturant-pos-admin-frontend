...
import LoyaltyClient from '@/features/loyalty-points/components/loyalty-client';
import { searchParamsCache } from '@/lib/searchparams';
import { SearchParams } from 'nuqs/server';

export const metadata = {
  title: 'Dashboard: Loyalty Points'
};
...
export default async function Page(props: pageProps) {
  ...
  try {
    ...
    console.error('Error fetching loyalty customers server-side:', err);
  }

  return (
    <PageContainer scrollable={false}>
      <div className="flex flex-1 flex-col space-y-4 w-full">
        <LoyaltyClient users={initialUsers} totalItems={totalItems} />
      </div>
    </PageContainer>
  );
}
