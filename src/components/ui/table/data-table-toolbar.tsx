...
function DataTableToolbarTextFilter<TData>({
  ...
  const lastPushedValue = React.useRef(filterValue ?? '');

  // Sync with external resets (e.g. clear filters)
  React.useEffect(() => {
    if (filterValue !== lastPushedValue.current) {
      setValue(filterValue ?? '');
      lastPushedValue.current = filterValue ?? '';
    }
  }, [filterValue]);

  // When debounced value changes, update the filter
  React.useEffect(() => {
    if (debouncedValue !== filterValue) {
      lastPushedValue.current = debouncedValue;
      column.setFilterValue(debouncedValue);
    }
  }, [debouncedValue, column, filterValue]);
